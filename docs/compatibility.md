...
Release `0.1.0` (Git tag `v0.1.0`; not published to the npm registry). The [AI contract](../ai/README.md) and [changelog](../CHANGELOG.md) describe it; runtime/color data is unchanged from candidate.0. Source `schemaVersion=1.0.0` is independent from package version; unknown schema versions fail. Public strings (`product.*`, brand IDs, token IDs) are stable identities, not display labels or positions. Persist series bindings in the consumer's own data; do not rebuild them from a sorted/filtered index. Theme affects values only; brand is a separate lookup and cannot flow transitively into product.series.

In 0.x releases, consumers pin an exact version (`exact:` in SwiftPM, `--save-exact` in npm). After stable release, removals/renames, changes to semantic meaning, field units, required platforms or API signatures require a major version. Additive IDs are minor; bug fixes that preserve meaning may be patch, but any visible color change needs review, contrast checks and native/Web parity evidence. Never reuse a retired series ID for another meaning. Deprecate aliases before removing them; no silent fallback aliasing at runtime.
...
## Explicitly deferred
Independent Apple NativeDesignKit repository, UI, Web React adapter, business migration, persisted assignment store, simulator/device/UI/visual/a11y certification, shared-ci policy/exception/downgrade enforcement, full recursive layer resolver/hooks/anti-corruption machinery, online branch/ruleset required enforcement and publication. CI file existence is not CI execution or merge protection. Native planning belongs to the independent Apple scope, not this color candidate's manifest/source/schema/generator/API.
