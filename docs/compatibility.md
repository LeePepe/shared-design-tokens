...
Package is private `0.1.0-candidate.1`, never published by this task. The [AI contract](../ai/README.md) and [changelog](../CHANGELOG.md) describe this documentation/fixture candidate; runtime/color data is unchanged from candidate.0. Source `schemaVersion=1.0.0` is independent from package version; unknown schema versions fail. Public strings (`product.*`, brand IDs, token IDs) are stable identities, not display labels or positions. Persist series bindings in the consumer's own data; do not rebuild them from a sorted/filtered index. Theme affects values only; brand is a separate lookup and cannot flow transitively into product.series.
